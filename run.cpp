...
class WeightCursor {
 public:
  explicit WeightCursor(std::span<const float> payload) noexcept {
    payload_ = payload;
  }

  std::span<const float> take(std::size_t count, std::string_view weight_name) {
    if (count > remaining()) {
  ...
  void skip(std::size_t count, std::string_view region_name) {
    static_cast<void>(take(count, region_name));
  }
...
void read_checkpoint(std::span<const std::byte> bytes, Config& config,
  ...
  if (bytes.size() < sizeof(Config)) {
    throw std::runtime_error("checkpoint is too small to contain Config");
  }
  // 文件开头就是 7 个 int32_t 组成的 Config。
  std::memcpy(&config, bytes.data(), sizeof(Config));

  // vocab_size 的符号编码了 Wcls 是否与 embedding 共享。
  if (config.vocab_size == 0 ||
      config.vocab_size == std::numeric_limits<std::int32_t>::min()) {
    throw std::runtime_error("invalid vocab_size in checkpoint");
  }
  ...
  // Config 后面必须是连续的 FP32 权重。
  const auto payload_bytes = bytes.subspan(sizeof(Config));
  if (payload_bytes.size() % sizeof(float) != 0) {
    throw std::runtime_error("checkpoint weight payload is not float-aligned");
  }

  const auto* payload_data =
      reinterpret_cast<const float*>(payload_bytes.data());

  const std::size_t payload_count = payload_bytes.size() / sizeof(float);

  WeightCursor cursor{std::span<const float>{payload_data, payload_count}};
  ...
  const std::size_t seq_len = static_cast<std::size_t>(config.seq_len);
  const std::size_t head_size = config.head_size();
  const std::size_t kv_dim = config.kv_dim();

  weights.token_embedding_table =
      cursor.take(vocab_size * dim, "token_embedding_table");
  weights.rms_att_weight = cursor.take(n_layers * dim, "rms_att_weight");
  weights.wq = cursor.take(n_layers * dim * dim, "wq");
  weights.wk = cursor.take(n_layers * dim * kv_dim, "wk");
  weights.wv = cursor.take(n_layers * dim * kv_dim, "wv");
  weights.wo = cursor.take(n_layers * dim * dim, "wo");
  weights.rms_ffn_weight = cursor.take(n_layers * dim, "rms_ffn_weight");
  weights.w1 = cursor.take(n_layers * dim * hidden_dim, "w1");
  weights.w2 = cursor.take(n_layers * hidden_dim * dim, "w2");
  weights.w3 = cursor.take(n_layers * dim * hidden_dim, "w3");
  weights.rms_final_weight = cursor.take(dim, "rms_final_weight");

  // 上游旧格式保留了两张 RoPE 表。
  // 当前 forward() 动态计算 sin/cos，因此这里只跳过。
  const std::size_t rope_table_size = seq_len * head_size / 2;

  cursor.skip(rope_table_size, "legacy_rope_real");
  cursor.skip(rope_table_size, "legacy_rope_imag");

  if (shared_weights) {
    weights.wcls = weights.token_embedding_table;
  } else {
    weights.wcls = cursor.take(vocab_size * dim, "wcls");
  }
}
...
class Transformer {
 public:
  explicit Transformer(const std::filesystem::path& checkpoint_path)
    ...
    read_checkpoint(checkpoint_.bytes(), config_, weights_);
    state_ = RunState(config_);
  }
...
void matmul(std::span<float> output, std::span<const float> matrix,
  ...
  if (rows > std::numeric_limits<std::size_t>::max() / columns) {
    throw std::invalid_argument("matmul dimensions overflow");
  }
  if (output.size() != rows || input.size() != columns ||
  ...
  for (std::size_t row = 0; row < rows; row++) {
    float sum = 0.0F;
    const auto matrix_row = matrix.subspan(row * columns, columns);

    for (std::size_t column = 0; column < columns; column++) {
      sum += matrix_row[column] * input[column];
    }
    output[row] = sum;
  }
}
...
std::span<float> Transformer::forward(std::int32_t token,
  ...
  const auto embedding = weights_.token_embedding_table.subspan(
      static_cast<std::size_t>(token) * dim, dim);
  std::copy(embedding.begin(), embedding.end(), x.begin());
  ...
  for (std::size_t layer = 0; layer < n_layers; layer++) {
    ...
    kernel::matmul(q, weights_.wq.subspan(layer * dim * dim, dim * dim), xb,
                   dim, dim);
    kernel::matmul(current_key,
                   weights_.wk.subspan(layer * dim * kv_dim, dim * kv_dim), xb,
                   kv_dim, dim);
    kernel::matmul(current_value,
                   weights_.wv.subspan(layer * dim * kv_dim, dim * kv_dim), xb,
                   kv_dim, dim);
    ...
    kernel::matmul(xb2, weights_.wo.subspan(layer * dim * dim, dim * dim), xb,
                   dim, dim);
    ...
    kernel::matmul(
        hb, weights_.w1.subspan(layer * hidden_dim * dim, hidden_dim * dim), xb,
        hidden_dim, dim);
    kernel::matmul(
        hb2, weights_.w3.subspan(layer * hidden_dim * dim, hidden_dim * dim),
        xb, hidden_dim, dim);
    ...
    kernel::matmul(
        xb, weights_.w2.subspan(layer * dim * hidden_dim, dim * hidden_dim), hb,
        dim, hidden_dim);
  ...
  kernel::matmul(logits, weights_.wcls, x, vocab_size, dim);
  return logits;
}
